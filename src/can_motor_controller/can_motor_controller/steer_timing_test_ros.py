...
# Change one POS_KP value per run. Keep POS_KI at zero while tuning KP.
POS_KP = 0.0
POS_KI = 0.0
...
class SteerTimingNode(Node):
    ...
    def _state_callback(self, message: JointState) -> None:
        try:
            ...
            if len(message.effort) > index:
                self.current_a = float(message.effort[index])
    ...
    def configure_motor_gains(self) -> None:
        print(f"Setting controller motor gains: POS_KP={POS_KP:g}, POS_KI={POS_KI:g}")
        ...
        future = self.param_client.set_parameters(
            [
                Parameter("steer_pos_kp", Parameter.Type.DOUBLE, float(POS_KP)),
                Parameter("steer_pos_ki", Parameter.Type.DOUBLE, float(POS_KI)),
            ]
        )
        ...
        # The controller applies pending motor parameters in its next CAN timer tick.
        deadline = time.monotonic() + 0.5
        while rclpy.ok() and time.monotonic() < deadline:
            rclpy.spin_once(self, timeout_sec=0.05)
        print("Gain request accepted. Check controller log for hardware write acknowledgement.")
    ...
    def move_and_time(self, target_deg: float, label: str, measured: bool) -> MoveResult:
        ...
        while rclpy.ok() and time.monotonic() - move_start <= MOVE_TIMEOUT_SEC:
            ...
            peak_current = max(peak_current, abs(self.current_a))
            self.trace.append(
                TracePoint(
        ...
        print(
            ...
            f"peak current={peak_current:.2f} A, samples={len(positions)}"
        )
        if measured:
    ...
    def save_results(self) -> None:
        ...
        stem = f"steer_ros_KP_{POS_KP:g}_KI_{POS_KI:g}_{self.timestamp}"
        ...
        axes[3].plot(times, [p.q_current_a for p in self.trace], color="tab:orange")
        axes[3].set_ylabel("Q current (A)")
        axes[3].set_xlabel("Run time (s)")
        for axis in axes:
            axis.grid(True)
        figure.suptitle(f"ROS steering timing: KP={POS_KP:g}, KI={POS_KI:g}")
        figure.tight_layout()
