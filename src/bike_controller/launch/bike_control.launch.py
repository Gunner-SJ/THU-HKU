"""LQR balance launch (RC).

Kickstands: kickstand_speed node (measured speed only; not RC / not balance_executor).
"""
...
def _plot_on_shutdown(context):
    ...
    print(f"[launch] plotting + opening latest CSV in {LOG_DIR} ...")
    subprocess.run(
        [
...
def generate_launch_description():
    ...
    return LaunchDescription([
        ...
        Node(
            package='bike_teleop',
            executable='teleop_supervisor',
            name='teleop_supervisor',
            output='screen'
        ),
        ...
        Node(
            ...
            parameters=[{
                ...
                'roll_rate_filter_alpha': 0.5,
                'balance_enable_speed': 1.0,
