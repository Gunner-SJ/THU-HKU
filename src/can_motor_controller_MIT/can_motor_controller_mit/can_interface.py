...
class CANInterface:
    ...
    def receive_mit_response(
        self,
        dev_addr: int,
        timeout: Optional[float] = None,
    ) -> can.Message:
        """Receive the seven-byte F1-compatible response to an MIT command."""
        if self._bus is None:
            raise CANCommunicationError("CAN bus is not open")
        ...
        valid_ids = {
            dev_addr,
            HOST_PREFIX | dev_addr,
            MIT_MODE_PREFIX | dev_addr,
            MIT_MODE_PREFIX | HOST_PREFIX | dev_addr,
        }
        while time.monotonic() < deadline:
            ...
            if msg is None:
                continue
            if (
                msg.arbitration_id in valid_ids
                and len(msg.data) == 7
                and msg.data[0] == 0xF1
            ):
                return msg
        raise CANTimeoutError(
            f"No MIT status response from motor 0x{dev_addr:02X} "
            f"within {timeout_value}s"
        )

    def send_mit_and_receive(
        self,
        dev_addr: int,
        data: bytes,
        timeout: Optional[float] = None,
    ) -> can.Message:
        """Send one MIT control frame and receive its immediate status response."""
        if len(data) != 8:
            raise ValueError(f"MIT control payload must be 8 bytes, received {len(data)}")
        self.send_mit_command(dev_addr, data)
        return self.receive_mit_response(dev_addr, timeout=timeout)
