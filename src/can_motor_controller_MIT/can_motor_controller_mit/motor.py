...
from .protocol import (
    ...
    parse_fault_code,
    pack_mit_control_payload, decode_mit_status_payload,
...
class Motor:
    ...
    def __init__(
        ...
        self._mit_pos_max = 95.5
        self._mit_vel_max = 45.0
        self._mit_torque_max = 18.0
    ...
    def mit_configure(
        ...
        data = self._cmd(
            Command.MIT_CONFIG,
            pack_2u(raw_pos) + pack_2u(raw_vel) + pack_2u(raw_t),
        )
        config = MITConfig(
        ...
        self.set_mit_limits_local(config.pos_max, config.vel_max, config.t_max)
        return config

    def mit_read_config(self) -> MITConfig:
        """Read MIT mode configuration."""
        data = self._cmd(Command.MIT_CONFIG)
        config = MITConfig(
            pos_max=unpack_2u(data, 1) * 0.1,
            vel_max=unpack_2u(data, 3) * 0.01,
            t_max=unpack_2u(data, 5) * 0.01,
    ...
    def set_mit_limits_local(
        self,
        pos_max_rad: float,
        vel_max_rad_s: float,
        t_max_nm: float,
    ) -> None:
        """Set host-side scaling to values already configured in the motor."""
        if pos_max_rad <= 0 or vel_max_rad_s <= 0 or t_max_nm <= 0:
            raise ValueError("MIT position, velocity, and torque limits must be positive")
        self._mit_pos_max = float(pos_max_rad)
        self._mit_vel_max = float(vel_max_rad_s)
        self._mit_torque_max = float(t_max_nm)
    ...
    def mit_read_status(self) -> MITStatus:
        ...
        return decode_mit_status_payload(
            data,
            self._mit_pos_max,
            self._mit_vel_max,
            self._mit_torque_max,
        )
    ...
    def mit_control(
        ...
        """
        payload = pack_mit_control_payload(
            position_rad,
            velocity_rad_s,
            kp,
            kd,
        ...
        response = self._can.send_mit_and_receive(
            self._dev_addr,
            payload,
            timeout=self._timeout,
        )
